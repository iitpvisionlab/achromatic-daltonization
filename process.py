...
def run_ma(dataloader, device, out_path, opt_name, eps, 
           num_epochs, sign_g, avg_v_fill, sim_func, sim_func_name, 
           px, cvdtype, n_data, h_max, w_max, 
           method, thresh, LR):
    ...
    for inputs, file_names in dataloader:
        ...
        # N, H, W
        sign_guide = get_sign_guide(inputs, device, sign_g, 
                                    sim_func_name=sim_func_name, 
                                    CVD_type=cvdtype)

        dw_x = get_dw(inputs, device, sim_func, sign_guide, px, 2, method, thresh, avg_v_fill)
        dw_y = get_dw(inputs, device, sim_func, sign_guide, px, 1, method, thresh, avg_v_fill)
        ...
        for i in range(len(inputs)):
            ...
            append_to_dic_ma(dataset_processed, file_names[i], original, 
                             original_sim, losses[:, i], x[i])
        # savings
...
def append_to_dic_ma(dictionary, file_name, original, 
                     original_sim, loss_graph, wmark):
...
def run_o_ma(dataloader, device, ma_path, out_path, opt_name, eps, 
           num_epochs, sim_func, sim_func_name, px, n_data, h_max, w_max, LR):
    dataset_processed = {"file_name":[], "original":[], "original_sim":[],
                          "loss_graph": [], "watermark": []};
    ...
    for inputs, file_names in dataloader:
        ...
        # watermark
        if not ma_path is None:
            x = torch.tensor(init_watermark[curr_b:curr_b+b], requires_grad=True, device=device)
        else:
            x = torch.ones((b, h, w), requires_grad=True, device=device)
        if opt_name == 'Adam':
            optimizer = torch.optim.Adam([x], lr=LR)
